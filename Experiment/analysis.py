...
for file in os.listdir(os.getcwd()+"/Experiment/data"):
    ...
    if file.endswith(".csv"):
        ...
        total_fiorentini_result = np.append(total_fiorentini_result, fiorentini_result)
        ...
        # visual result
        x = np.array([0.005,0.01,0.15])
        values = range(len(x))

        fy = np.array([((fiorentini_time<v)&(fiorentini_result!="exception")).sum() for v in x])
        ...
        plt.plot(values,cppy,marker="o",label="fiorentini")
